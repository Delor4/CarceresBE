...
def send_email_notifications():
    with app.app_context():
        ...
        subscribers_to_notify = session.query(Subscription) \
            .filter(Subscription.notification_sended == False) \
            .filter(Subscription.end > now).filter(Subscription.end < reminder_period) \
        ...
        with mail.connect() as conn:
            for note in subscribers_to_notify:
                ...
                message = "Witaj, %s.\n" \
                          ...
                          "\n\nWiadomość wysłana automatycznie. Prosimy nie odpowiadać na ten email." % user.client.name
