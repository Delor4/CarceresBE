...
class ListResource(ResourceBase):
    ...
    def finalize_post_req(self, model):
        ...
        return (
            ...
            self.make_response_headers(
                location=url_for(self.model_name, id=model.id, _external=True)
            ),
        )
    ...
    def get_paginated_list(self, models, sort_params, sort_arg, start, limit):
        ...
        if start < 1 or count < start:
            abort(
                404,
                message=f"Pagination start outside allowed values. Expected: 1 - {count}. Provided: {start}",
            )
        if limit < 1:
            abort(
                404,
                message=f"Pagination limit outside allowed values. Expected more than 0. Provided: {limit}",
            )
        ...
        if start != 1:
            ...
            obj["previous"] = self._add_arg(
                resource_url + "?start=%d&limit=%d" % (start_copy, limit_copy), sort_arg
            )
        # make next url
        if start + limit <= count:
            start_copy = start + limit
            obj["next"] = self._add_arg(
                resource_url + "?start=%d&limit=%d" % (start_copy, limit), sort_arg
            )
